...
class GameVue(Scene):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if not pressed_mouse_buttons[0]:
                if self.building is None:
                    ...
                    self.building.coords = (
                                                   ...
                                                   - Point(self.screen_width, self.screen_height) // 2
            ...
            if self.clicking:
                ...
                self.building_moved = (
                                      ...
                                      ) // self.cell_pixel_size != Point.origin()
    ...
    def render_map(self):
        ...
        chunk_br = (
                chunk_tl
                + Point(
            Perlin.CHUNK_SIZE * self.cell_pixel_size,
            Perlin.CHUNK_SIZE * self.cell_pixel_size,
        )
                - camera_offset
        )
        ...
        cells_tl = Point(
            ceil(margin_tl.x / self.cell_pixel_size),
            ceil(margin_tl.y / self.cell_pixel_size),
        )
        cells_br = Point(
            ceil(margin_br.x / self.cell_pixel_size),
            ceil(margin_br.y / self.cell_pixel_size),
        )
        chunks_tl = Point(
        ...
        cells_tl_signed = Point(
            chunk_tl.x / self.cell_pixel_size, chunk_tl.y / self.cell_pixel_size
        )
        cells_br_signed = Point(
            (self.screen_width - chunk_br.x) / self.cell_pixel_size,
            (self.screen_height - chunk_br.y) / self.cell_pixel_size,
        )
        cells_size = Point(
        ...
        for i in range(0, cells_size.x):
            for j in range(0, cells_size.y):
                ...
                self.screen.blit(
                    ...
                    (
                        i * self.cell_pixel_size - camera_offset.x,
                        j * self.cell_pixel_size - camera_offset.y,
                    ),
                )
        ...
        if self.building is not None:
            ...
            for point in self.building.points:
                absolute_point = (
                                 ...
                                 ) * self.cell_pixel_size - camera_offset
                pygame.draw.rect(
                    self.screen,
                    self.colors[
                        (
            ...
            for y in range(tl_chunk.y, tl_chunk.y + chunks_size.y + 1):
                ...
                if actual_chunk_occupied_coords is not None:
                    for point in actual_chunk_occupied_coords:
                        ...
                        if (
                                struct is not None
                                and struct.structure_type == StructureType.BUILDING
                        ):
                            absolute_point = (
                                    (point - camera_cell) * self.cell_pixel_size
                                    + screen_center
                                    - camera_offset
                            )
                            pygame.draw.rect(
