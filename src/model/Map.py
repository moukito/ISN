...
class Map:
    ...
    def try_generate_ore(
        ...
        if random.random() < treshold:
            ...
            for i in range(search_area_size * search_area_size):
                chunk_ores = self.ores.get(
                    Point(
                        chunk_coords.x + values[i // search_area_size],
                        chunk_coords.y + values[i % search_area_size],
                    ),
                    None,
                )
                if chunk_ores != None:
            ...
            if ores_count < ores_count_treshold:
                ...
                while not found and i < len(ore.points):
                    if (
                            self.occupied_coords.get(
                                ore.points[i] + absolute_chunk_origin, None
                            )
                            is not None
                    ):
                        found = True
    ...
    def try_place_structure(self, structure):
        ...
        while (
                i < l
                and self.occupied_coords.get(center + structure.points[i], None) is None
        ):
            i += 1
